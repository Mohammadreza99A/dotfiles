...
local function create_button(icon_name, action_name, color, onclick, icon_size,
                             icon_margin)
    ...
    button:connect_signal("mouse::enter",
                          function() action:set_text(action_name) end)
...
local function launch(args)
    ...
    local onlock = args.onlock or
                       function() awful.spawn.with_shell("i3lock-fancy") end
    ...
    w:set_bg(bg_color)
    if #phrases > 0 then
        phrase_widget:set_markup('<span color="' .. text_color ..
                                     '" size="20000">' ..
                                     phrases[math.random(#phrases)] .. "</span>")
    end
    ...
    w:setup{
        {
            ...
            {
                {
                    create_button("refresh-cw", "Reboot (r)", accent_color,
                                  onreboot, icon_size, icon_margin),
                    create_button("lock", "Lock (k)", accent_color, onlock,
                                  icon_size, icon_margin),
                    create_button("moon", "Suspend (u)", accent_color,
                                  onsuspend, icon_size, icon_margin),
                    create_button("log-out", "Log Out (l)", accent_color,
                                  onlogout, icon_size, icon_margin),
                    create_button("power", "Power Off (s)", accent_color,
                                  onpoweroff, icon_size, icon_margin),
                    id = "buttons",
                    spacing = 8,
