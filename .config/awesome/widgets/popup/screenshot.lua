...
local snap_area_button = create_button.button_with_label("Selection",
                                                         beautiful.icon_crop)
local snap_full_button = create_button.button_with_label("Screen",
                                                         beautiful.icon_screen)
local snap_window = create_button.button_with_label("Window",
                                                    beautiful.icon_window)
snap_area_button.bg = beautiful.bg_focus
snap_full_button.bg = beautiful.bg_normal
snap_window.bg = beautiful.bg_dark

snap_area_button:connect_signal("button::press", function(self, _, _, button)
    if button == 1 then
        snap_area = 'area'
        self.bg = beautiful.bg_focus
        snap_full_button.bg = beautiful.bg_dark
        snap_window.bg = beautiful.bg_dark
    end
end)
...
local btn_delay_num = function()
    ...
    btn_minus:connect_signal("button::press", function(_, _, _, button)
        if button == 1 then
            if delay_sec < 1 then return end
            delay_sec = delay_sec - 1
            value_text:set_text(delay_sec)
        end
    end)
...
local row_top = wibox.widget {
    {button_capture, spacing = dpi(4), layout = wibox.layout.fixed.horizontal},
    nil,
    btn_close,
...
for s in screen do s.maim_popup = maim_popup end

local hide_maim_popup = function()
    for s in screen do s.maim_popup.visible = not s.maim_popup.visible end
end

local show_maim_popup = function()
    for s in screen do s.maim_popup.visible = true end
end
...
local take_screen_shot = function()
    ...
    if snap_area == 'area' then
        command = "maim -s -d " .. delay_sec .. " " .. screen_shot_dir ..
                      file_name
    elseif snap_area == 'screen' then
        command = "maim -d " .. delay_sec .. " " .. screen_shot_dir .. file_name
    elseif snap_area == "window" then
        command = "maim -d " .. delay_sec .. " " ..
                      "-i $(xdotool getactivewindow) " .. screen_shot_dir ..
                      file_name
    end
    ...
    local open_file_cb = function()
        awful.spawn.with_shell('xdg-open ' .. screen_shot_dir .. file_name,
                               false)
    end

    local open_dir_cb = function()
        awful.spawn.with_shell('xdg-open ' .. screen_shot_dir, false)
    end

    local delete_file_cb = function()
        awful.spawn.with_shell('gio trash ' .. screen_shot_dir .. file_name,
                               false)
    end
...
button_capture:connect_signal("button::press", function(_, _, _, button)
    if button == 1 then take_screen_shot() end
end)

btn_close:connect_signal("button::press", function(_, _, _, button)
    if button == 1 then hide_maim_popup() end
end)
...
screen_shot_button:connect_signal("button::press", function(self, _, _, button)
    if button == 1 then display_popup() end
end)
