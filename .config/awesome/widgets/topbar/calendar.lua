...
-- calendar.lua --> Calendar widget in awesome wm
--
...
local function worker(user_args)
  ...
  local function decorate_cell(widget, flag, date)
    ...
    local default_bg =
      (weekday == 0 or weekday == 6) and calendar_themes[theme].weekend_day_bg or calendar_themes[theme].bg
    local ret =
      wibox.widget {
      {
        {
  ...
  function calendar_widget.toggle()
    if popup.visible then
      ...
      elseif placement == "top_right" then
        awful.placement.top_right(popup, {margins = {top = 30, right = 80}, parent = awful.screen.focused()})
